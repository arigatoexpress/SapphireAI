...
from cloud_trader.prompt_engineer import (
    PromptBuilder,
    ResponseValidator,
    PromptContext,
)
...
```"""
    ...
    def test_extract_content_various_formats(self, response_validator):
        ...
        response1 = {"response": '{"direction": "BUY", "confidence": 0.7, "rationale": "Test rationale"}', "content": ""}
        content1 = response_validator._extract_content(response1)
        assert content1 is not None
        assert "direction" in content1

        # Test format 2: content field
        response2 = {"content": '{"direction": "SELL", "confidence": 0.6, "rationale": "Test rationale"}'}
        content2 = response_validator._extract_content(response2)
        assert content2 is not None

        # Test format 3: choices array
        response3 = {"choices": [{"message": {"content": '{"direction": "HOLD", "confidence": 0.5, "rationale": "Test rationale"}'}}]}
