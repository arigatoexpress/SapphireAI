...
async def test_select_and_execute_position_holding(fallback_selector):
    ...
    with patch.object(
        fallback_selector, "_position_holding", new_callable=AsyncMock
    ) as mock_position_holding:
...
async def test_select_and_execute_no_position(fallback_selector):
    ...
    with (
        patch("random.choice", return_value="low_frequency_momentum") as mock_choice,
        patch.object(
            fallback_selector, "_low_frequency_momentum", new_callable=AsyncMock
        ) as mock_low_freq,
    ):
    ...
    with (
        patch("random.choice", return_value="reduced_symbol_scanning") as mock_choice,
        patch.object(
            fallback_selector, "_reduced_symbol_scanning", new_callable=AsyncMock
        ) as mock_reduced_scan,
    ):
