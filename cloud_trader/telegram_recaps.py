...
class TelegramRecapService:
        ...
        try:
            ...
            if agent_results:
                ...
                for agent in agent_results[:6]:
                    agent_name = (
                        agent.agent_id.replace("-", " ").title() if agent.agent_id else "Unknown"
                    )
            ...
            if include_chart:
                ...
                if chart_url:
                    await self.bot.send_photo(
                        chat_id=self.chat_id, photo=chart_url, caption="📈 Performance Chart"
                    )
    ...
    async def _generate_performance_chart(self, hours: int) -> Optional[str]:
        ...
        try:
            ...
            # Volume chart
            ax1.plot(
                hours_list, volume_list, color="#00ffff", linewidth=2, marker="o", markersize=4
            )
