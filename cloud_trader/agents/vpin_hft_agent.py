...
from ..partial_exits import PartialExitStrategy, PositionExitPlan, ExitSignal
...
class VpinHFTAgent:
    ...
    def calculate_vpin(self, tick_data_batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        timing_precision = len(microsecond_deltas) / len(tick_data_batch) if microsecond_deltas else 0
    ...
    def ml_classify_trade_direction(self, tick: Dict[str, Any], market_context: Dict[str, Any]) -> int:
        """
    ...
    def _extract_trade_features(self, tick: Dict[str, Any], market_context: Dict[str, Any]) -> List[float]:
        """Extract features for ML classification."""
        return [
            tick.get('price', 0),
            tick.get('volume', 0),
    ...
    def calculate_quote_imbalance(self, order_book: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        bid_volume = sum(level['volume'] for level in order_book.get('bids', [])[:5])  # Top 5 levels
        ...
        if len(self.quote_imbalance_history) >= 10:
            ...
            stdev = statistics.stdev(self.quote_imbalance_history) if len(self.quote_imbalance_history) > 1 else 1.0
    ...
    def generate_hybrid_signal(self, vpin_data: Dict[str, Any], quote_imbalance: Dict[str, Any],
                             correlations: List[Dict[str, Any]], regime: Optional[RegimeMetrics] = None) -> Dict[str, Any]:
        """
        ...
        buy_signal = (vpin_cdf['z_score'] > buy_thresholds['vpin_z'] and
                     quote_imbalance['z_score'] > buy_thresholds['qi_z'] and
                     correlation_score > buy_thresholds['corr_sum'])

        sell_signal = (vpin_cdf['z_score'] < sell_thresholds['vpin_z'] and
                      quote_imbalance['z_score'] < sell_thresholds['qi_z'] and
                      correlation_score < sell_thresholds['corr_sum'])

        signal_strength = abs(vpin_cdf['z_score']) + abs(quote_imbalance['z_score']) + abs(correlation_score)
    ...
    def _get_regime_adjusted_thresholds(self, regime: Optional[RegimeMetrics]) -> Tuple[Dict[str, float], Dict[str, float]]:
    ...
    async def calculate_adaptive_position_size(self, signal_data: Dict[str, Any],
    ...
    def get_regime_based_position_size(self, regime: Optional[RegimeMetrics], base_position_size: float) -> float:
    ...
    async def create_position_with_exit_plan(self, symbol: str, entry_price: float,
        ...
        try:
            ...
            # Execute entry trade
            entry_result = await self._execute_entry_trade(
                symbol, entry_price, optimal_size, side
            )
    ...
    async def monitor_and_execute_exits(self, symbol: str, current_price: float) -> List[Dict[str, Any]]:
        """
    ...
    async def force_position_close(self, symbol: str, close_price: Optional[float] = None,
                                 reason: str = "manual_close") -> bool:
    ...
    def _generate_position_recommendations(self, position_status: Dict) -> List[str]:
        ...
        if remaining_size > 0:
            if holding_time > 300:  # 5 minutes
                recommendations.append("Consider reducing position size due to extended holding time")
    ...
    async def _execute_entry_trade(self, symbol: str, price: float, size: float, side: str) -> Dict[str, Any]:
    ...
    async def _execute_exit_trade(self, exit_signal: ExitSignal) -> Dict[str, Any]:
        ...
        # In a real implementation, this would interface with the exchange
        logger.info(f"Simulated exit trade: {exit_signal.symbol} {exit_signal.exit_size} at {exit_signal.exit_price or 'market'} ({exit_signal.reason})")
    ...
    async def _start_position_monitoring(self, symbol: str, exit_plan: PositionExitPlan):
        ...
        logger.info(f"Started monitoring position {symbol} with {len(exit_plan.exit_levels)} exit levels")

    async def _send_position_close_notification(self, symbol: str, position_status: Dict, reason: str):
        """Send notification about position closure."""

        # In a real implementation, this would send Telegram alerts
        logger.info(f"Position {symbol} closed - Reason: {reason}, P&L: {position_status.get('total_pnl', 0):.2f}")
