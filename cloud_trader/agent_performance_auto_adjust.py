...
class PerformanceAutoAdjuster:
    ...
    def record_trade_outcome(self, agent_id: str, pnl: float, win: bool, confidence: float,
        ...
        current_max_dd = self.performance_windows[agent_id][PerformanceMetric.MAX_DRAWDOWN].get_average() or 0.05
        if pnl < 0:
    ...
    def _analyze_and_adjust(self, agent_id: str, current_time: int) -> List[AgentAdjustment]:
        ...
        adjustments.extend(self._analyze_confidence_threshold(agent_id, state, performance_data, current_time))
        adjustments.extend(self._analyze_position_size(agent_id, state, performance_data, current_time))
        adjustments.extend(self._analyze_capital_allocation(agent_id, state, performance_data, current_time))
        adjustments.extend(self._analyze_feature_flags(agent_id, state, performance_data, current_time))

        return adjustments

    def _analyze_confidence_threshold(self, agent_id: str, state: AgentState,
                                    performance_data: Dict, current_time: int) -> List[AgentAdjustment]:
        ...
        if abs(new_threshold - state.confidence_threshold) > 0.01:  # Minimum change threshold
            adjustments.append(AgentAdjustment(
                agent_id=agent_id,
    ...
    def _analyze_position_size(self, agent_id: str, state: AgentState,
                             performance_data: Dict, current_time: int) -> List[AgentAdjustment]:
    ...
    def _analyze_capital_allocation(self, agent_id: str, state: AgentState,
                                  performance_data: Dict, current_time: int) -> List[AgentAdjustment]:
    ...
    def _analyze_feature_flags(self, agent_id: str, state: AgentState,
                             performance_data: Dict, current_time: int) -> List[AgentAdjustment]:
            ...
            for feature in features_to_disable:
                if state.features_enabled.get(feature, False):
                    adjustments.append(AgentAdjustment(
                        ...
                        timestamp_us=current_time,
                        performance_trigger={'win_rate': win_rate, 'accuracy': accuracy, 'feature': feature}
                    ))
    ...
    def _apply_adjustment(self, adjustment: AgentAdjustment) -> None:
        ...
        logger.info(f"Applied adjustment to {adjustment.agent_id}: {adjustment.adjustment_type.value} "
        ...
        for agent_id, state in self.agent_states.items():
            if agent_id in self.performance_windows:
                ...
                agent_overview[agent_id] = {
                    ...
                    'active_features': sum(1 for enabled in state.features_enabled.values() if enabled),
                    'last_adjustment': state.last_adjustment.adjustment_type.value if state.last_adjustment else None
                }
...
async def get_performance_auto_adjuster(consensus_engine: Optional[AgentConsensusEngine] = None) -> PerformanceAutoAdjuster:
