...
class VotingService:
    ...
    async def submit_vote(
        ...
        existing_votes = self.db.collection('daily_votes')\
            .where('uid', '==', uid)\
            .where('symbol', '==', symbol)\
            .where('date', '==', today)\
            .limit(1)\
    ...
    async def get_crowd_sentiment(self, symbol: str) -> Dict[str, Any]:
        ...
        votes = self.db.collection('daily_votes')\
            .where('symbol', '==', symbol)\
            .where('date', '==', today)\
            .stream()
    ...
    async def score_predictions(self) -> Dict[str, Any]:
        ...
        for vote_doc in votes:
            ...
            if is_correct:
                ...
                if confidence > 0.8:
                    ...
                    logger.info(f"🎯 {uid} HIGH-CONF CORRECT: {symbol} {prediction} ({confidence:.0%}) +100pts")
                else:
        ...
        logger.info(f"📊 Scored {scored_count} predictions: {correct_count} correct, {points_awarded_total} points awarded")
