...
from .agents import BacktestAgent, create_backtest_agent, BacktestAgentDecision
...
class BacktestEngine:
    ...
    async def run_backtest(
        ...
        logger.info(f"Backtest {run_id} completed: {results.total_return:.2%} return, {results.total_trades} trades")
        return results
    ...
    def _calculate_results(
        ...
        annualized_return = ((final_capital / initial_capital) ** (1 / years) - 1) if years > 0 else 0.0
        ...
        if len(returns) > 0 and np.std(returns) > 0:
            sharpe_ratio = float(np.mean(returns) / np.std(returns) * np.sqrt(252 * 24))  # Hourly to annual
