...
const MetricCard: React.FC<MetricCardProps> = ({ label, value, delta, icon, accent = 'teal', footer }) => {
    ...
    return (
        <div className={`group sapphire-panel p-6 before:absolute before:inset-0 before:bg-gradient-to-br before:opacity-60 before:transition-opacity before:duration-200 ${accentMap[accent]}`}>
            ...
            <div className="relative flex items-start justify-between">
                <div className="transition-transform duration-200 group-hover:translate-y-[-1px]">
                    <p className="text-xs uppercase tracking-[0.35em] text-brand-muted/80 transition-colors duration-200 group-hover:text-brand-muted">{label}</p>
                    <div className="mt-2 text-3xl font-semibold text-brand-ice transition-transform duration-200 group-hover:scale-105">{value}</div>
                    {delta && (
                        <div className="mt-2 flex items-center gap-2 text-xs font-medium text-brand-muted">
                            ...
                            <span className="transition-colors duration-200 group-hover:text-brand-muted/90">{delta.label ?? 'vs previous'}</span>
                        </div>
                    )}
                </div>
                {icon && <span className="text-3xl text-brand-muted/70 transition-transform duration-200 group-hover:scale-110 group-hover:rotate-3">{icon}</span>}
            </div>
            {footer && <div className="relative mt-4 text-xs text-brand-muted transition-colors duration-200 group-hover:text-brand-muted/80">{footer}</div>}
        </div>
    );
};
