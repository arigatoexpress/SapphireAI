import { collection, doc, getDoc, increment, limit, onSnapshot, orderBy, query, runTransaction, serverTimestamp, setDoc, updateDoc, addDoc, } from 'firebase/firestore';
...
const sanitizeDisplayName = (input) => {
    if (!input)
        return 'Anon Builder';
...
export const ensureMemberProfile = async (user) => {
...
export const recordCheckIn = async (user) => {
    const db = ensureFirestore();
...
export const subscribeSentiment = (user, callback) => {
    const db = ensureFirestore();
...
export const castVote = async (user, vote) => {
    const db = ensureFirestore();
    const sentimentDoc = doc(db, SENTIMENT_DOC_PATH);
    const memberDoc = doc(db, COLLECTION_MEMBERS, user.uid);
    const today = getDateKey();
    await runTransaction(db, async (transaction) => {
        ...
        let sentiment = sentimentSnap.data();
...
export const subscribeLeaderboard = (callback, limitSize = 10) => {
    const db = ensureFirestore();
    const leaderboardQuery = query(collection(db, COLLECTION_MEMBERS), orderBy('points', 'desc'), limit(limitSize));
...
export const subscribeCommunityComments = (callback, limitSize = 50) => {
    const db = ensureFirestore();
    const commentsQuery = query(collection(db, COLLECTION_COMMENTS), orderBy('createdAt', 'desc'), limit(limitSize));
...
export const addCommunityComment = async (user, message) => {
    const db = ensureFirestore();
...
export const isRealtimeCommunityEnabled = () => firestoreEnabled;
